...
from cte
where City in ('JAIPUR', 'MUMBAI', 'VARANASI'); -- This query will tell that if those cities are still profitable or not, will have to change 'City' everytime to get deeper insights
...
limit 3;  -- This is the main query
...
order by City
	...
	from cte
    order by waste desc;  -- This is for cities who have the largest gap between copies printed and net circulation overall (from 2019 to 2024)
...
(select City,
    ...
    from cte_2
    where City in ('JAIPUR', 'MUMBAI', 'VARANASI');  -- how the gap has changed over time
 ...
 order by standard_ad_category, `year`; -- make a bar chart for separate ad_ categories and show thee ad revenue in bars
 ...
 group by city
 order by ad_revenue desc;  -- find top cities with highest ad revenue, and there is no co relation between both columns
							-- also check the co relation through visulaization. 
...
from cte
order by digital_readiness desc, pilot_engagement asc; -- Kanpur had highest digital readiness ( how prepared a population (or city) is to adopt and benefit from digital technologies)
 ...
 group by City
    ...
    from cte
    order by Circulation_ROI desc; -- which cities have highest circulation ROI
