...
with cte as
(
select fa.ad_category as category_id,
	fa.year, fa.ad_revenue_in_inr, da.standard_ad_category as category_name
from fact_ad_revenue  as fa
left join dim_ad_category as da
on fa.ad_category = da.ad_category_id
), cte_2 as
(
select `year`, 
           category_name, 
...
(select city_name,
	case
		when sum(decline_print)  =5 then "Yes" else "No"
        end as is_declining_print,
	case
		when sum(declining_ad_revenue) =5 then "Yes" else "No"
        end as is_declining_ad_revenue,
	case 
		when sum(decline_print)  =5 and sum(declining_ad_revenue) =5 then "Yes" else "No"
